# SuperClaw ETH Perps — Copy-Trade Skill

A SuperClaw skill that copy-trades a single, centrally-curated **ETH** perpetual agent on Hyperliquid onto **your own** account. Install it in SuperClaw, complete a short setup, and the skill mirrors the curated agent's ETH positions for you. You stay in control and can pause or stop anytime.
...
- Follows **one curated ETH agent** (selected centrally by SuperClaw — you do not pick the trader) and mirrors its position changes onto your Hyperliquid account.
- **Locked to ETH** — the skill only ever places ETH orders.
...
```
install https://github.com/superpowerdevk/superclaw-perps-eth
```
...
2. **Fund it with USDC** — deposit USDC into that wallet's Hyperliquid account (USDC on Arbitrum or HyperEVM). Perps are USDC-margined — you never deposit ETH itself.
3. **Authorize trading** — open the authorize link with that wallet and sign **Agent + Builder** (no funds move, just permission).
4. **Send your wallet address** — reply with the address you used, and the skill starts copying ETH trades.
...
- `allowed_coins` — the skill's asset lock (this skill: **ETH** only)
