...
description: Manage the SuperClaw ETH perpetual copy-trading service on Hyperliquid. Use when the user wants to start/stop/check the follow service, generate an agent wallet, create/fund the isolated sub-account, complete authorization, view the curated agent's track record, view trade history/stats, or sync to the latest curated agent. The followed agent trades ETH perps only, is selected centrally by the SuperClaw admin, and is resolved automatically at service start - the user never picks a trader.
---

# SuperClaw ETH Perps Copy-Trade — Interaction Flow (EN)

> For SuperClaw / OpenClaw skill development. Describes the Bot's full in-conversation behavior.
> This skill copy-trades a single curated **ETH-perp** agent on Hyperliquid into the user's own **isolated sub-account** (`sc-eth`), so ETH risk is ring-fenced from other SuperClaw skills.
...
After installing the SuperClaw ETH Perps Copy-Trade skill, the user can — via conversation — automatically follow a centrally-curated **ETH-perp** agent on Hyperliquid, mirrored into their own **isolated sub-account** (`sc-eth`). The followed agent is **not chosen by the user**; it is resolved automatically from the official remote pointer at service start/resume.

The flow has four stages:

1. Wallet setup (connect main wallet + generate Agent Wallet + sign & register)
2. Sub-account setup (create the isolated `sc-eth` sub-account + fund it)
...
### 1. Single asset: ETH only
`allowed_coins` is locked to `["ETH"]`. The curated agent trades ETH perps; the skill only ever places ETH orders. Never widen the whitelist.

### 2. Isolated sub-account (`sc-eth`)
Trades are routed into a dedicated Hyperliquid **sub-account** named `sc-eth` under the user's master wallet, so ETH margin and PnL are isolated from the user's other SuperClaw skills.

- After wallet authorization, the service attempts to **auto-create** `sc-eth` at start (`subaccount create`). If Hyperliquid rejects agent-signed creation, the Bot guides the user to create a sub-account named `sc-eth` in the Hyperliquid UI and set its address: `config set subaccount_address 0x...`.
- The user must then **fund `sc-eth`** by transferring USDC from their main Hyperliquid account into the sub-account (Hyperliquid Portfolio → Transfer). This is the funding step — the Bot must tell the user funds go into the `sc-eth` sub-account, not the main account and not an on-chain transfer to an address.
- `service start` will refuse to run if `require_subaccount` is set and `sc-eth` has no address — it will print setup guidance instead of silently trading on the master account.
- Check status anytime with `subaccount show`.

Setup order for this skill: **wallet setup → create + fund `sc-eth` → configure parameters → start.**
...
1. **Coin scope**: locked to **ETH** only via the `allowed_coins` whitelist (`["ETH"]`). The curated agent trades ETH perps; do not widen the whitelist for this skill.
