---
name: superclaw-perps-eth
description: Manage the SuperClaw ETH perpetual copy-trading service on Hyperliquid. Use when the user wants to start/stop/check the follow service, generate an agent wallet, complete authorization, view the curated agent's track record, view trade history/stats, or sync to the latest curated agent. The followed agent trades ETH perps only, runs on a dedicated Hyperliquid account, is selected centrally by the SuperClaw admin, and is resolved automatically at service start - the user never picks a trader.
---
...
> This skill copy-trades a single curated **ETH-perp** agent on Hyperliquid. It runs on its **own dedicated Hyperliquid account** — a **fresh wallet funded with USDC**, separate from every other SuperClaw skill (required: HL caps agents per account, so each skill needs its own wallet).
...
After installing the SuperClaw ETH Perps Copy-Trade skill, the user can — via conversation — automatically follow a centrally-curated **ETH-perp** agent on Hyperliquid. For isolation, this skill runs on its **own dedicated Hyperliquid account** (a fresh wallet used only for ETH). The followed agent is **not chosen by the user**; it is resolved automatically from the official remote pointer at service start/resume.
...
> SuperClaw ETH Perps — let's get you set up 🚀
> ~3 minutes. The agent copy-trades ETH for you. You stay in control and can stop anytime.
>
...
> Sign in to Hyperliquid with that new wallet — **email, OKX Wallet, MetaMask,** or any option they show. Then deposit **USDC** (the only thing you add — never ETH itself). Use **USDC on Arbitrum** or **USDC on HyperEVM**.
> → https://app.hyperliquid.xyz
>
> ### 3️⃣ Authorize trading
> Open the link below with that same wallet and sign **Agent + Builder**. This lets the bot place ETH trades for you — no funds move, just permission.
> → https://moss.site/hyperliquid/authorize/<AGENT_WALLET>
>
> ### 4️⃣ Send me your wallet address
> Reply with the `0x…` address from step 1, and I'll start copying ETH trades.
>
...
- NEVER tell the user to deposit ETH — collateral is always **USDC** (Arbitrum or HyperEVM).
...
> - **"show my position"** — your open ETH trade right now
