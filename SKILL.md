...
Open Hyperliquid with that wallet and deposit **USDC** — that's the only thing you add (never ETH itself; perps are USDC-margined). Use **USDC on Arbitrum** or **USDC on HyperEVM**.
...
Reply with the `0x…` address of the wallet you just used, and I'll start copying ETH trades.
...
- Collateral is always **USDC** — never tell the user to deposit ETH.
